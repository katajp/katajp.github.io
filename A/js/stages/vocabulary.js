...
function updateVocabSetupInfo(){
  ...
  el.textContent=pool.length?`${pool.length} words selected`:"No words match this filter.";
  const btn=document.getElementById("vocabStartBtn");
  if(btn)btn.disabled=pool.length===0;
}

/* ---------------- Session lifecycle ---------------- */
function startNewVocabQuiz(){
  const pool=filterVocabulary(setupSelJlpt,setupSelCategories);
...
function getWeakVocab(){
  ...
  return VOCABULARY.filter(v=>weakSet.has(v.word));
}
...
function newVocabQuestion(){
  ...
  enrichWord(correct).then(info=>{
    if(info&&info.example&&info.example.jp){
      ...
      ex.innerHTML=`<div style="font-family:var(--font-jp);">${info.example.jp}</div>${info.example.en?`<div>${info.example.en}</div>`:""}`;
      area.appendChild(ex);
    }
  });
}
